...
{	
	...
	function __construct()
	{
		...
		if(isset($array[0]) && is_array($array[0]) && isset($config["show_dir"]) && $config["show_dir"] == true)
		{
			...
			{
				$dirData=$config["directory"].DIRECTORY_SEPARATOR.$dirData;
				?>
		...
		{  
			...
			{
			$realFileData=$config["directory"].DIRECTORY_SEPARATOR.$fileData;
				?>
				...
				{
					?>
						<img src="<?php echo $this->iconImage($realFileData); ?>" />
					<?php
				}
				?>
				</td>
				<td class="synFileTd">
				<a href="<?php echo $realFileData; ?>"><?php echo $this->showFileExtension($fileData); ?></a>
				</td>
				<?php
				if(isset($config["show_last_modified"]) && $config["show_last_modified"] == true)
				{
				...
				if(isset($config["show_size"]) && $config["show_size"] == true)
				{
					...
					<td class="synFileTd" align="right">
						<?php echo $this->fileSizeCalc($realFileData); ?>
					</td>
					<?php
				}
				?>
	...
	*/ 
	function synDir()
	{
		...
		$iterator = new DirectoryIterator($config["directory"]);
